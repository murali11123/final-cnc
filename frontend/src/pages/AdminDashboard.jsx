...
const AdminDashboard = () => {
  ...
  const stats = {
    ...
    templeDesigns: designs.filter(d => d.category === 'Temple Designs').length,
  ...
  const handleSaveDesign = async (e) => {
    ...
    try {
      if (isEditModalOpen && selectedDesign) {
        // Edit Design
        await updateAdminDesign(selectedDesign._id, formData, (progressEvent) => {
          const percent = Math.round((progressEvent.loaded * 100) / progressEvent.total);
          setUploadProgress(percent);
        });
        toast.success('Design updated. AI embedding recalculating in background.');
      } else {
        // Create Design
        ...
        toast.success('Design created. AI embedding calculating in background.');
      }
      ...
      toast.error(error.response?.data?.message || 'Failed to save design specifications.');
    } finally {
      setIsSaving(false);
      setUploadProgress(0);
    }
  ...
  const handleOpenEditModal = (design) => {
    ...
    setUploadFile(null);
    setUploadPreview(design.imageUrl.startsWith('/uploads/') ? `http://localhost:5000${design.imageUrl}` : design.imageUrl);
    setUploadProgress(0);
  ...
  const handleRegenerateAI = async (designId) => {
    ...
    try {
      ...
      toast.success('Regeneration worker launched in background.', { id: toastId });
      loadData();
    } catch (err) {
  ...
  const handleBulkReindex = async () => {
    const confirm = window.confirm('Are you sure you want to regenerate AI embeddings for all 48+ designs? This might load your system CPU for a minute.');
    if (!confirm) return;

    const toastId = toast.loading('Re-indexing database embeddings. Please wait...');
    try {
      const response = await rebuildAllEmbeddings();
      toast.success(`Re-indexing completed. Success: ${response.success}, Failed: ${response.failed}`, { id: toastId, duration: 6000 });
      loadData();
    } catch (err) {
  ...
  const categories = ['2D Wall Panels', 'Temple Designs', 'Custom CNC', 'Wooden Crafts'];
      ...
      <aside className="w-full md:w-64 bg-slate-900 text-slate-400 shrink-0 flex flex-col justify-between p-6">
        <div>
          ...
          <div className="flex items-center gap-2 mb-8 select-none">
            <svg className="w-7 h-7 text-brand-secondary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
          ...
          <nav className="space-y-1">
            <button
              ...
              className={`flex items-center gap-3 w-full py-2.5 px-4 rounded-xl text-sm font-semibold transition ${
                activeTab === 'dashboard' ? 'bg-brand-primary text-white' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-3 w-full py-2.5 px-4 rounded-xl text-sm font-semibold transition ${
                activeTab === 'designs' ? 'bg-brand-primary text-white' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
              <Layers size={18} />
            ...
            <button
              ...
              className={`flex items-center justify-between w-full py-2.5 px-4 rounded-xl text-sm font-semibold transition ${
                activeTab === 'messages' ? 'bg-brand-primary text-white' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
        ...
        <div className="mt-12 border-t border-slate-800 pt-6">
          <div className="text-xs text-slate-500 mb-2 truncate">Logged: <strong>{admin?.username}</strong></div>
          <button
      ...
      <main className="flex-grow p-6 md:p-10 overflow-x-hidden">
        ...
        <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">
              {activeTab === 'dashboard' && 'Visual analysis and summary totals.'}
              {activeTab === 'designs' && 'Upload, edit, delete CNC carving designs.'}
              {activeTab === 'messages' && 'Read and flag inquiries from customers.'}
            </p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[300px] gap-3">
            ...
            <span className="text-sm font-semibold text-slate-400">Loading catalog modules...</span>
          </div>
        ) : (
          <>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Total Designs</span>
                      <span className="text-3xl font-extrabold text-brand-dark">{stats.totalDesigns}</span>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Active Visible</span>
                      <span className="text-3xl font-extrabold text-emerald-600">{stats.activeDesigns}</span>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">AI Ready</span>
                      <span className="text-3xl font-extrabold text-brand-secondary">{stats.aiReady}</span>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">New Messages</span>
                      <span className="text-3xl font-extrabold text-red-500">{stats.unreadMessages}</span>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm text-center">
                      <div className="text-2xl font-black text-brand-dark mb-1">{stats.wallPanels}</div>
                      <div className="text-xs font-bold text-slate-400">2D Wall Panels</div>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm text-center">
                      <div className="text-2xl font-black text-brand-dark mb-1">{stats.templeDesigns}</div>
                      <div className="text-xs font-bold text-slate-400">Temple Designs</div>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm text-center">
                      <div className="text-2xl font-black text-brand-dark mb-1">{stats.customCnc}</div>
                      <div className="text-xs font-bold text-slate-400">Custom CNC</div>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm text-center">
                      <div className="text-2xl font-black text-brand-dark mb-1">{stats.woodenCrafts}</div>
                      <div className="text-xs font-bold text-slate-400">Wooden Crafts</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 shadow-sm">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 mb-1">AI Embeddings Engine Status</h4>
                      <p className="text-xs text-slate-500 leading-relaxed mb-3">
                        The AI Design Search matches visual profiles by referencing stored vector embeddings. If any design shows a "Failed" AI status in the tables below, it will not appear in the image search results.
                      </p>
                      <div className="flex flex-wrap gap-4 text-xs font-bold font-mono text-slate-600">
                        <span className="text-brand-secondary">Ready: {stats.aiReady}</span>
                        <span className="text-orange-500">Processing: {stats.aiProcessing}</span>
                        <span className="text-red-500">Failed: {stats.aiFailed}</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-600">
                      {filteredDesigns.map((design) => {
                        const imgSource = design.imageUrl.startsWith('/uploads/') 
                          ? `http://localhost:5000${design.imageUrl}` 
                          : design.imageUrl;
                        return (
                          <tr key={design._id} className="hover:bg-slate-50/70 transition-colors">
                            {/* Image */}
                            <td className="py-3 px-6">
                              <div className="w-12 h-12 bg-slate-50 rounded-lg border border-slate-200 flex items-center justify-center p-1 overflow-hidden">
                                <img src={imgSource} alt="Design thumbnail" className="max-w-full max-h-full object-contain" />
                              </div>
                            </td>
                            {/* Name */}
                            <td className="py-3 px-6 font-bold text-brand-dark max-w-[150px] truncate">{design.name}</td>
                            {/* Code */}
                            <td className="py-3 px-6 font-mono font-bold text-brand-primary">{design.code}</td>
                            {/* Category */}
                            ...
                            <td className="py-3 px-6">
                              ...
                              >
                                {design.active ? <Eye size={12} /> : <EyeOff size={12} />}
                                <span>{design.active ? 'Visible' : 'Hidden'}</span>
                              </button>
                            </td>
                            {/* AI Status */}
                            <td className="py-3 px-6">
                              <span className={`inline-flex items-center gap-1 font-bold py-1 px-2.5 rounded-full text-[10px] ${
                                ...
                                {design.aiStatus === 'Ready' && <CheckCircle size={10} />}
                                {design.aiStatus === 'Processing' && <Clock size={10} className="animate-spin" />}
                                {design.aiStatus === 'Failed' && <XCircle size={10} />}
                            ...
                            <td className="py-3 px-6">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  onClick={() => handleOpenDeleteConfirm(design._id)}
                ...
                {messages.map((message) => (
                  <div
                    ...
                    className={`bg-white border rounded-3xl p-6 shadow-sm hover:shadow-md transition-all duration-300 ${
                      message.status === 'unread' ? 'border-l-4 border-l-brand-primary border-slate-200' : 'border-slate-200'
                    }`}
                  >
                    {/* Message Header */}
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {message.status === 'unread' && (
                            <span className="bg-red-500 text-white font-extrabold text-[8px] tracking-widest uppercase px-2 py-0.5 rounded">NEW</span>
                          )}
                          {message.status === 'replied' && (
                            <span className="bg-green-500 text-white font-extrabold text-[8px] tracking-widest uppercase px-2 py-0.5 rounded">REPLIED</span>
                          )}
                        </div>
            ...
            <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200 overflow-hidden relative">
              ...
              <div className="p-6 md:p-8">
                <h3 className="text-lg font-extrabold text-brand-dark mb-6">
                  {isEditModalOpen ? 'Edit Design Specifications' : 'Add New CNC Design'}
                </h3>

                <form onSubmit={handleSaveDesign} className="space-y-5">
                  {/* File Upload zone with preview */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Design Image *</label>
                    <div className="flex items-center gap-4">
                      {uploadPreview ? (
                        <div className="w-20 h-20 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center p-1">
                          <img src={uploadPreview} alt="Image Preview" className="max-w-full max-h-full object-contain" />
                        </div>
                      ) : (
                      ...
                      <div className="flex-grow">
                        ...
                        <span className="text-[10px] text-slate-400 block mt-1">PNG, JPG, JPEG, or WEBP (Max 5MB)</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Design Name *</label>
                      <input
                        type="text"
                        required
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Design Code *</label>
                      <input
                        type="text"
                        required
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Price (INR, optional)</label>
                      <input
                        type="number"
        ...
        {isDeleteOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 text-center relative">
              ...
              <h3 className="text-base font-bold text-brand-dark mb-1">Confirm Design Deletion</h3>
              <p className="text-xs text-slate-500 leading-relaxed mb-6">
                Are you sure you want to delete this CNC design? This action removes the visual file and vector embedding index permanently and cannot be undone.
              </p>
